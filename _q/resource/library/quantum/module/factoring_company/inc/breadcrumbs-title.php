    ...
    <form action="" method="get">
      <div class="form-group">
        <select name="factoring_company_id" class="form-control" onchange="this.form.submit()">
          ...
          <?php
          for ($i = 1; $i <= $factoring_company_ALT_count ; $i++) { ?>
            
            <option value="<?= $factoring_company_ALT_id[$i] ?>"<?= $factoring_company_ALT_id[$i] == $_GET['factoring_company_id'] ? ' selected' : '' ?>><?= $factoring_company_ALT_status[$i] == 1 ? $factoring_company_ALT_name[$i] : '<span class="red">' . $factoring_company_ALT_name[$i] . '</span>' ?></option> <?php
          }
          ?>
