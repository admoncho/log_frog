...
if (Input::get('_controller_draft_load') == 'add_draft' || Input::get('_controller_draft_load') == 'update_draft') {
  ...
  if ($validation->passed()) {
    ...
    $drop_date_time = $drop_date_unit[2] . '-' . $drop_date_unit[0] . '-' . $drop_date_unit[1] . ' ' . $drop_time_unit[0] . ':' . $drop_time_unit[1] . ':00';
      ...
      # Add draft
	    $add = DB::getInstance()->query("INSERT INTO loader_load_draft (initial_rate, deadhead, broker_name_number, note, user_id) VALUES ('" . Input::get('initial_rate') . "', '" . Input::get('deadhead') . "', '" . htmlentities(Input::get('broker_name_number'), ENT_QUOTES) . "', '" . htmlentities(Input::get('note'), ENT_QUOTES) . "', " . $user->data()->id . ")");
        ...
        ");
   		...
   		# Update
   		$update_main = DB::getInstance()->query("UPDATE loader_load_draft SET 
