...
if(($_FILES["payment_confirmation_file"]["type"] == "application/pdf") && $_FILES["payment_confirmation_file"]["size"] < 7000000) {
  ...
  if ($_FILES["payment_confirmation_file"]["error"] > 0){
    ...
    # Display error
    Session::flash('loader_error', 'There was an error uploading the file');
  } else {
    ...
    if($validation->passed()) {
      ...
      if ($update->count()) {
        ...
        if (Input::get('payment_confirmation') == 2) {
          ...
          # Add note
          $insert = DB::getInstance()->query("INSERT INTO factoring_company_schedule_note (schedule_id, note, important, type, user_id) VALUES (" . $_GET['schedule_id'] . ", '" . htmlentities(Input::get('note'), ENT_QUOTES) . "', " . $important_note . ", 2, " . $user->data()->id . ")");
        }
        ...
        for ($i=1; $i <= $load_list_count ; $i++) {
          ...
          $insert = DB::getInstance()->query("INSERT INTO loader_load_note (load_id, note, important, type, user_id) VALUES (" . $load_list_load_id[$i] . ", 'Payment confirmation uploaded (" . $amount . " amount).', 0, 1, " . $user->data()->id . ")");
      ...
      Session::flash('loader_error', 'There was an error, data was missing.');
    }
  }
} else {
  Session::flash('loader_error', 'The file is not valid or it superseeds the size quota (max: 7MB)');
}
