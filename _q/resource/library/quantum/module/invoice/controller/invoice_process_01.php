...
/*
(revised) CHECK THIS DIFFERENT APPROACH
Get a list of all loads from the week criteria.
Based on that list get involved client list and charge from that point of view
Lot less code required, it'd require a join connection with loader_load > loader_entry > client_user tables
...
$week_loads_01 = DB::getInstance()->query("
	SELECT loader_load.entry_id, loader_load.load_id, loader_entry.driver_id, client_user.client_id FROM loader_load 
...
foreach ($week_loads_01->results() as $week_loads_01_data) {
	...
	$insert = DB::getInstance()->query("
		...
		VALUES (" . $week_loads_01_data->client_id . ", " . $week_loads_01_data->load_id . ", '" . $week_loads_01_data->driver_id . "')
	");
}
