  			...
  			<div class="panel panel-default<?= $_POST['add_load'] ? ' hidden' : '' ?>">
				  ...
				  <div class="table-responsive<?= $_GET['new_draft'] ? ' hidden' : '' ?>">
					...
					if ($_GET['draft_rate_con'] && $draft_has_rate_con[1]) { ?>
						...
						<embed 
							src="<?= $_SESSION['HtmlDelimiter'] ?>files/draft-rate-confirmation/<?= $_GET['draft_rate_con'] ?>.pdf?r=<?= date('Gis') ?>" 
							width="100%" 
							height="1000px"> <?php
					}
					?>
			...
			<div class="main-box-body clearfix">
				...
				if (isset($_POST['add_load'])) { ?>
					...
					if ($_POST['draft_id'] && file_exists($draft_rate_con_path . $_POST['draft_id'] . '.pdf')) { ?>
						...
						<embed 
							src="<?= $_SESSION['HtmlDelimiter'] ?>files/draft-rate-confirmation/<?= $_POST['draft_id'] ?>.pdf?r=<?= date('Gis') ?>" 
							width="100%" 
							height="1000px"> <?php
					}
				} else {
