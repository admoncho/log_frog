					...
					<div class="col-sm-12 col-md-4">
						...
						<p class="text-center lead">
							<small>
								...
								<?= $load_reference[1] ? '<b>Ref #: </b><span class="red">' . $load_reference[1] . '</span>' : '' ?>
					...
					<div class="panel-body">
						...
						if ($_GET['rate_confirmation'] && $rate_confirmation_exists[1]) {
						 	...
						 	<form action="" method="post">
						 		...
						 		<input type="hidden" name="path" value="<?= str_replace('_q', 'files/', $_SESSION['ProjectPath']); ?>">
						 		<input type="hidden" name="file_name" value="rate-confirmation-<?= $load_entry_id[1] . '-' . $load_load_id[1] ?>.pdf">
		  					<input type="hidden" name="token" value="<?= $csrfToken ?>">
						 	</form>

							<embed id="rate-confirmation" src="<?= $_SESSION['HtmlDelimiter'] ?>files/rate-confirmation-<?= $load_entry_id[1] . '-' . $load_load_id[1] ?>.pdf?r=<?= date('Gis') ?>" width="100%" height="1001px"> <?php
						...
						if ($_GET['bol'] && $bol_exists[1]) {
							...
							<embed id="bol" src="<?= $_SESSION['HtmlDelimiter'] ?>files/bol-<?= $load_entry_id[1] . '-' . $load_load_id[1] ?>.pdf?r=<?= date('Gis') ?>" width="100%" height="1001px"> <?php
						...
						if ($_GET['raw_bol'] && $raw_bol_exists[1]) {
							...
							<embed id="raw-bol" src="<?= $_SESSION['HtmlDelimiter'] ?>files/raw-bol-<?= $load_entry_id[1] . '-' . $load_load_id[1] ?>.pdf?r=<?= date('Gis') ?>" width="100%" height="1001px"> <?php
						...
						if ($_GET['payment_confirmation'] && $payment_confirmation_exists[1]) {
							...
							<embed id="payment-confirmation" src="<?= $_SESSION['HtmlDelimiter'] ?>files/payment-confirmation-<?= $load_entry_id[1] . '-' . $load_load_id[1] ?>.pdf?r=<?= date('Gis') ?>" width="100%" height="1001px"> <?php
						...
						if ($_GET['quickpay_invoice'] && $quickpay_invoice_exists[1]) {
						 	...
						 	<form action="" method="post">
						 		...
						 		<input type="hidden" name="_controller_loader_file" value="delete_file">
						 		<input type="hidden" name="path" value="<?= str_replace('_q', 'files/', $_SESSION['ProjectPath']) . 'quickpay-invoices/'; ?>">
						 		<input type="hidden" name="file_name" value="invoice-<?= $load_entry_id[1] . '-' . $load_load_id[1] ?>.pdf">
		  					<input type="hidden" name="token" value="<?= $csrfToken ?>">
						 	</form>

							<embed id="invoice" src="<?= $_SESSION['HtmlDelimiter'] ?>files/quickpay-invoices/invoice-<?= $load_entry_id[1] . '-' . $load_load_id[1] ?>.pdf?r=<?= date('Gis') ?>" width="100%" height="1001px"> <?php
