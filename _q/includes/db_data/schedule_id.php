...
$factoring_company_schedule_load_counter = 1;
...
if ($factoring_company_schedule_load_count) {
	...
	foreach ($factoring_company_schedule_load->results() as $factoring_company_schedule_load_data) {
		...
		$load_list_billing_status[$factoring_company_schedule_load_counter] = $factoring_company_schedule_load_data->billing_status;
		$factoring_company_schedule_load_counter++;
	}
	...
	$load_list = DB::getInstance()->query("SELECT loader_load.entry_id, loader_load.broker_id, loader_load.load_id, loader_load.load_number, loader_load.line_haul, loader_load.miles, loader_load.reference, loader_load.billing_status, loader_load.user_id, loader_load.commodity, loader_load.load_lock, loader_load.load_status, loader_entry.driver_id, loader_entry.data_id FROM loader_load RIGHT JOIN loader_entry ON loader_load.entry_id=loader_entry.data_id WHERE loader_load.load_id IN (" . implode(', ', $load_id) . ") ORDER BY data_id ASC");
	...
	if ($load_list_count) {
		...
		foreach ($load_list->results() as $load_list_data) {
			...
			$load_list_reference[$load_list_counter] = html_entity_decode($load_list_data->reference);
	...
	# Deduct sum of other charges to sum of line hauls
	$schedule_amount = $line_haul_sum - $other_charges_sum;

	# Get emails for cc field, these are managers (owners or owner/operators)
	$schedule_load_manager_email = DB::getInstance()->query("SELECT * FROM user_e_profile_client_user WHERE user_id IN (" . implode(', ', $load_list_driver_id_array) . ")");
	...
	foreach ($schedule_load_manager_email->results() as $schedule_load_manager_email_data) {
		...
		if ($schedule_load_manager_email_data->user_type == 2) {
			...
			$driver_manager_id[] = $_QU_e_email[$schedule_load_manager_email_data->user_manager];
		} else {

			# Owner/operator
			$owner_id[] = $_QU_e_email[$schedule_load_manager_email_data->user_id];
		}
	}
}
...
for ($i = 1; $i <= $factoring_company_schedule_load_count ; $i++) { 
	...
	# Declare file name
	$pre_invoice_name[$i] = strtolower(str_replace([' & ', ' '], ['-', '-'], ($schedule_invoice_number[$i]) . '-' . $limbo_client_company_name_by_data_id[$client_assoc_factoring_company_client_id] . '-' . $broker_co_name_did[$load_list_broker_id[$i]] . '.pdf'));
