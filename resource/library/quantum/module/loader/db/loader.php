...
if ($load_count) {
	...
	foreach ($load->results() as $load_data) {
		...
		file_exists($_SESSION['ProjectPath'] . '/files/bol-' . $load_entry_id[$i] . '-' . $load_load_id[$i] . '.pdf') ? $bol_exists[$i] = 1 : '';
		file_exists($_SESSION['ProjectPath'] . '/files/rate-confirmation-' . $load_entry_id[$i] . '-' . $load_load_id[$i] . '.pdf') ? $rate_confirmation_exists[$i] = 1 : '';
		file_exists($_SESSION['ProjectPath'] . '/files/raw-bol-' . $load_entry_id[$i] . '-' . $load_load_id[$i] . '.pdf') ? $raw_bol_exists[$i] = 1 : '';
		file_exists($_SESSION['ProjectPath'] . '/files/payment-confirmation-' . $load_entry_id[$i] . '-' . $load_load_id[$i] . '.pdf') ? $payment_confirmation_exists[$i] = 1 : '';
		file_exists($_SESSION['ProjectPath'] . '/files/quickpay-invoices/invoice-' . $load_entry_id[$i] . '-' . $load_load_id[$i] . '.pdf') ? $quickpay_invoice_exists[$i] = 1 : '';
	...
	if ($draft_list_count) {
		...
		foreach ($draft_list->results() as $draft_list_data) {
			...
			foreach ($draft_item->results() as $draft_item_data) {
				...
				$draft_item_final_rate_1[$i] = $draft_item_data->initial_rate;
				$draft_item_deadhead[$i] = number_format($draft_item_data->deadhead, 2);
				$draft_item_weight[$i] = $draft_item_data->weight;
				$draft_item_broker_name_number[$i] = html_entity_decode($draft_item_data->broker_name_number);
				$draft_item_broker_email[$i] = html_entity_decode($draft_item_data->broker_email);
				$draft_item_note[$i] = html_entity_decode($draft_item_data->note);
				$draft_item_loaded[$i] = $draft_item_data->loaded;
