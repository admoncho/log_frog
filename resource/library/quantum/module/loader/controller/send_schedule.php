...
for ($i = 0; $i < $to_items_count ; $i++) {
	...
	if ($valid == false) {
		...
		Session::flash('loader_error', 'All email addresses must be valid.');
		Redirect::to('schedule?schedule_id=' . $_GET['schedule_id'] . '&fee_option=' . $_GET['fee_option']);
	}
...
if(!$mail->send()) {
  ...
  // echo 'Mailer Error: ' . $mail->ErrorInfo;
	Session::flash('loader_error', 'There was an error sending the schedule, please try again.') ;
	Redirect::to('schedule?schedule_id=' . $_GET['schedule_id'] . '&fee_option=' . $_GET['fee_option']);
} else {
	...
	if ($load_list_count == $update_count) {
		...
		$update_counter->count() ? Session::flash('loader', 'Schedule sent successfully!') . Redirect::to('schedule?schedule_id=' . $_GET['schedule_id']) : Session::flash('loader_error', 'There was an error updating data.') ;
	}

	Session::flash('loader', 'Schedule sent successfully!') . Redirect::to('schedule?schedule_id=' . $_GET['schedule_id']);
}
